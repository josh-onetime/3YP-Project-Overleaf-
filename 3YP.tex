...
\begin{document}
	...
	\section{Hardware}
		...
		\subsection{Locomotion}
			...
			For the duration of this calculation, we will assume we are in the smallest possible pipe diameter $D = 609.6$ mm, and we will assume that the curvature radius $r_c = \frac{3}{2} D = 914.4$ mm.
			As our robot diameter $D_r \nless 2D \sin \left( 45^\circ \right) - D = 252.5$ mm we can use the following equation \textsuperscript{\cite{roh2005differential}}:
			\begin{align}
				l_{max} &= 2 \sqrt{4D^2 - \left( D + D_r\right) ^ 2}
